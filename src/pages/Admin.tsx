...
export default function Admin() {
  ...
  useEffect(() => {
    ...
    const unsubApprovals = onSnapshot(
      ...
      (snap) => {
        setApprovals(
          snap.docs.map(
            (d: any) => ({ id: d.id, ...d.data() }) as CompletionApproval,
          ),
        );
      },
    );
  ...
  const handleToggleSignups = async () => {
    try {
      ...
      await setDoc(
        doc(db, "settings", "global"),
        { allowSignups: newValue },
        { merge: true },
      );
      setAllowSignups(newValue);
      showAlert(
        "Settings Updated",
        newValue ? "System signups enabled." : "System signups disabled.",
        "success",
      );
    } catch (error) {
  ...
  const saveAdminSignature = async () => {
    ...
    try {
      ...
      let signatureData = "";
      try {
        const svgData = adminSigCanvas.current
          ?.getSignaturePad()
  ...
  const onTaskSubmit = async (data: TaskFormData) => {
    ...
    try {
      ...
      showAlert(
        "Error",
        `Error saving task: ${e.message || "Unknown error"}`,
        "error",
      );
    } finally {
      setSubmittingTask(false);
  ...
  const handleUpdateRole = async (
    targetId: string,
    newRole: AdminMember["role"],
    targetDisplayName: string,
    targetCurrentRole: AdminMember["role"],
  ) => {
    ...
    if (currentUserRole === "admin") {
      if (targetCurrentRole === "developer") {
        showAlert(
          "Error",
          "Administrators cannot modify Developer accounts.",
          "error",
        );
        return;
      }
      ...
      if (newRole === "developer") {
        showAlert(
          "Error",
          "Administrators cannot promote accounts to Developer.",
          "error",
        );
        return;
      }
    ...
    try {
      await updateDoc(doc(db, "admins", targetId), {
        role: newRole,
        updatedAt: serverTimestamp(),
      });
      setMembers((prev) =>
      ...
      showAlert(
        "Success",
        `${targetDisplayName}'s role updated to ${newRole.replace("_", " ").toUpperCase()}`,
        "success",
      );
    } catch (err) {
  ...
  const handleDeleteTask = async (id: string) => {
    ...
    if (
      !window.confirm(
        "Delete this task? All records linked to it will remain but orphaned from the schedule.",
      )
    )
      return;
    try {
  ...
  const sendCompletionEmail = async (student: StudentProgress) => {
    try {
      ...
      const latestRecord = [...student.records].sort((a, b) =>
      ...
      const pdfBase64 = await generateObligationPDF({
        ...
        academicYear:
          (latestRecord as any)?.academicYear ||
          `A.Y. ${new Date().getFullYear()} - ${new Date().getFullYear() + 1}`,
        studentSignature: (latestRecord as any)?.studentSignature,
        approverName:
          student.approval?.approverName ||
          adminDoc?.displayName ||
          "Authorized Representative",
        approverRole:
          student.approval?.approverRole ||
          (adminDoc?.role === "admin" ? "OSA Admin" : "OSA Staff"),
        approverSignature:
          adminDoc?.signature || student.approval?.approverSignature,
        records: student.records
          .filter((r) => r.status === "verified")
          .map((r) => {
            // ADDED: Live signature lookup for the email attachment just like the preview
            const liveStaff = members.find(
              (m) => m.id === (r as any).verifiedById,
            );
      ...
      if (payloadSizeMB > 4.2) {
        showAlert(
          "Warning",
          `PDF is too large (${payloadSizeMB.toFixed(1)}MB) for the email server. Please generate the preview and email it manually.`,
          "warning",
        );
        return;
      }
      ...
      if (!response.ok) {
        if (response.status === 413) {
          throw new Error(
            "Payload Too Large: The PDF is too heavy to send via Vercel.",
          );
        }
      ...
      if (result.success) {
        showAlert(
          "Sent",
          `Completion form successfully sent to ${student.studentEmail}!`,
          "success",
        );
      }
    } catch (e: any) {
      ...
      showAlert(
        "Error",
        e.message || "Failed to send completion email automatically.",
        "error",
      );
    }
  };
  const handleApproveCompletion = async (student: StudentProgress) => {
    ...
    if (!adminDoc || !adminDoc.signature) {
      showAlert(
        "Action Required",
        "You must set up your digital signature in Settings before you can approve completions.",
        "warning",
      );
      setTab("settings");
      return;
    ...
    try {
      const approvalData = {
        ...
        approverRole:
          adminDoc.role === "admin" || adminDoc.role === "developer"
            ? "Head, Office of Student Affairs"
            : adminDoc.role === "staff"
              ? "OSA Staff"
              : "Student Assistant, OSA",
      ...
      showAlert(
        "Approved",
        `${student.studentName}'s completion has been approved.`,
        "success",
      );
      // Result handled by onSnapshot
    } catch (e) {
  ...
  const handleUndoApproval = async (
    ...
    const confirmUndo = window.confirm(
      `Are you sure you want to REVOKE the completion approval for ${studentName}? \n\nThis will return their status to 'Pending Verification'.`,
    );
    ...
    try {
      ...
      showAlert(
        "Revoked",
        `Approval for ${studentName} has been successfully undone.`,
        "success",
      );
      // Result handled by onSnapshot
    } catch (e) {
  ...
  const handleVerify = async (recordId: string, currentStatus: string) => {
    try {
      ...
      showAlert(
        "Success",
        `Attendance record has been ${newStatus}.`,
        "success",
      );
    } catch (e: any) {
      console.error("FULL ERROR OBJECT:", e);
      ...
      .reduce((acc: { [key: string]: StudentProgress }, r) => {
        if (!acc[r.studentNo]) {
          ...
          if (!acc[r.studentNo].approval) {
            acc[r.studentNo].approval = approvals.find(
              (ap) => ap.studentNo === r.studentNo,
            );
          }
        }
        ...
        acc[r.studentNo].hasNameMismatch =
          (acc[r.studentNo].allNames?.length || 0) > 1;

        return acc;
      }, {}),
  ).sort(
    (a, b) =>
      (b as StudentProgress).verifiedHours -
      (a as StudentProgress).verifiedHours,
  ) as StudentProgress[];
  ...
  const isTaskDone = (t: Task) => {
    ...
    return (
      ...
      taskRecords.every((r) => r.status === "verified")
    );
  };
  ...
  const studentProgress = studentProgressRaw.filter(
    (s) =>
    ...
    <div className="min-h-screen bg-[#1c1c1c] text-[#ededed] flex flex-col md:flex-row font-sans relative">
      ...
      >
        ...
        <nav className="flex-1 p-4 space-y-1">
          ...
          <div className="pt-4 mt-4 border-t border-[#2e2e2e]"></div>
        </nav>
        <div className="p-4 border-t border-[#2e2e2e]">
          <button
            onClick={logout}
            className="w-full flex items-center gap-3 px-3 py-2 rounded-md text-sm font-medium text-[#a1a1a1] hover:bg-[#2e2e2e] hover:text-[#ededed] transition-colors"
          >
            ...
            <div className="space-y-8 max-w-4xl">
              <div>
                ...
                <p className="text-[#a1a1a1] text-sm mt-1">
                  Publish available schedule slots for students.
                </p>
              </div>

              <form
                onSubmit={handleSubmit(onTaskSubmit)}
                className="bg-[#171717] border border-[#2e2e2e] p-3 md:p-4 rounded-lg grid grid-cols-1 md:grid-cols-4 gap-3 md:gap-4 text-xs"
              >
                <div className="space-y-1 md:col-span-4">
                  <label className="text-[10px] font-semibold uppercase text-[#a1a1a1]">
                    Activity / Task Title
                  </label>
                  <input
                ...
                <div className="md:col-span-4 flex justify-end gap-2">
                  {editingTask && (
                    <button
                      ...
                      className="px-3 py-1.5 text-[10px] font-bold border border-[#2e2e2e] rounded-md uppercase"
                    >
                      Cancel
                    </button>
                  )}
                  <button
                    ...
                    className={`${editingTask ? "bg-amber-500 border-amber-500" : "bg-[#3ecf8e] border-[#3ecf8e]"} text-black text-[10px] font-bold uppercase tracking-tight px-4 py-1.5 rounded-md flex items-center gap-1.5 transition-colors disabled:opacity-50 border`}
                  >
                    {submittingTask ? (
                  ...
                  <div className="hidden lg:block overflow-x-auto w-full">
                    <table className="min-w-full text-left text-sm whitespace-nowrap">
                      <thead className="bg-[#262626] border-b border-[#2e2e2e] text-[#a1a1a1] text-xs font-medium uppercase tracking-wider">
                        <tr>
                          <th className="px-3 py-2 text-[9px] font-bold uppercase tracking-wider">
                            Task Title
                          </th>
                          <th className="px-3 py-2 text-[9px] font-bold uppercase tracking-wider">
                            Schedule
                          ...
                          <th className="px-3 py-2 text-right text-[9px] font-bold uppercase tracking-wider">
                            Actions
                          </th>
                        </tr>
                      </thead>
                      <tbody className="divide-y divide-[#2e2e2e]">
                        {activeTasks.length === 0 ? (
                          <tr>
                            <td
                              colSpan={4}
                              className="px-4 py-4 text-center text-[#a1a1a1]"
                            >
                          ...
                          activeTasks.map((t) => (
                            <tr key={t.id} className="hover:bg-[#1c1c1c]">
                              <td className="px-3 py-1.5">
                                <div className="font-bold text-[#ededed] text-[11px]">
                                  {t.title}
                                </div>
                                <div className="text-[8px] text-[#a1a1a1] uppercase font-bold tracking-tight">
                                  {t.staffName}
                                </div>
                              </td>
                              <td className="px-3 py-1.5 text-[#a1a1a1] font-mono text-[9px]">
                                <div className="text-[#ededed] mb-0.5">
                                  {formatDate(t.date)}
                                </div>
                                <div>
                              ...
                              <td className="px-3 py-1.5">
                                <div className="flex items-center gap-1">
                                  ...
                                  <span className="text-[9px] font-bold text-[#ededed]">
                                    {
                                      records.filter(
                                        (r) =>
                                          r.taskTitle === t.title &&
                              ...
                              <td className="px-3 py-1.5 text-[#3ecf8e] font-bold text-[10px]">
                                  ...
                                  {records.some(
                                    ...
                                    <div className="flex items-center gap-1.5">
                                      {records
                                        ...
                                        <span className="text-[8px] text-[#3ecf8e] uppercase font-black px-1.5 py-0.5 bg-[#3ecf8e]/10 border border-[#3ecf8e]/30 rounded">
                                          Done
                                        </span>
                                      ) : (
                                        <span className="text-[8px] text-[#a1a1a1] uppercase font-bold italic border border-[#2e2e2e] px-1.5 py-0.5 rounded">
                                          Active
                                        </span>
                                      )}
                                      <button
                        ...
                        >
                          <div className="flex justify-between items-start gap-4">
                            <div className="min-w-0 flex-1">
                              <div className="font-bold text-[#ededed] break-words text-sm">
                                {t.title}
                              </div>
                              <div className="text-[10px] text-[#a1a1a1] uppercase font-bold mt-0.5">
                                {t.staffName}
                              </div>
                            </div>
                            <div className="text-right shrink-0">
                              <div className="text-[#3ecf8e] font-bold text-xs bg-[#3ecf8e]/10 px-2 py-0.5 rounded">
                          ...
                          <div className="bg-[#1c1c1c] p-2 rounded border border-[#2e2e2e]">
                            <div className="text-[#ededed] text-[10px] font-mono mb-1">
                              {formatDate(t.date)}
                            </div>
                            <div className="text-[#a1a1a1] text-[10px] font-mono">
                              {formatTime(t.startTime)} -{" "}
                              {formatTime(t.endTime)}
                            </div>
                          </div>
                            ...
                            {records.some(
                              ...
                              <div className="flex items-center gap-2">
                                {records
                                  ...
                                  <span className="text-[10px] text-[#3ecf8e] font-bold uppercase tracking-widest px-2 py-0.5 bg-[#3ecf8e]/5 border border-[#3ecf8e]/20 rounded">
                                    Done
                                  </span>
                                ) : (
                                  <span className="text-[10px] text-[#a1a1a1] uppercase font-bold italic px-2 py-0.5 bg-[#262626] rounded border border-white/5">
                                    In Progress
                                  </span>
                                )}
                                <button
              ...
              <div className="flex flex-col md:flex-row md:items-center justify-between gap-3">
                <div>
                  ...
                  <p className="text-[#a1a1a1] text-[10px] mt-0.5">
                    Review and verify student obligations.
                  </p>
                </div>
                <div className="relative w-full md:w-64">
                ...
                <div className="bg-[#171717] border border-amber-500/50 p-6 rounded-lg space-y-4">
                  <div className="flex justify-between items-center">
                    <h3 className="text-amber-500 font-bold flex items-center gap-2">
                      <Edit2 className="w-4 h-4" /> Editing Log for{" "}
                  ...
                  >
                    ...
                    <div className="space-y-1">
                      ...
                      <input
                        {...recordForm.register("studentEmail", {
                          required: true,
                        })}
                        className="w-full bg-[#1c1c1c] border border-[#2e2e2e] rounded p-2 text-sm focus:border-[#3ecf8e] outline-none"
                    ...
                    <div className="space-y-1">
                      ...
                      <select
                        {...recordForm.register("semester" as any)}
                        className="w-full bg-[#1c1c1c] border border-[#2e2e2e] rounded p-2 text-sm focus:border-[#3ecf8e] outline-none"
                      >
                    ...
                    <div className="space-y-1 md:col-span-2">
                      ...
                      <input
                        {...recordForm.register("academicYear" as any)}
                        className="w-full bg-[#1c1c1c] border border-[#2e2e2e] rounded p-2 text-sm focus:border-[#3ecf8e] outline-none"
                    ...
                    <div className="flex items-end md:col-span-4">
                      <button
                        type="submit"
                        className="w-full bg-amber-500 hover:bg-amber-600 text-black font-bold py-2 rounded text-sm transition-colors"
                      >
                        Save Changes
                      </button>
                    </div>
                    <div className="md:col-span-4 text-[10px] text-amber-500 italic mt-2">
                      * Credit hours are automatically calculated based on time
                      and capped at 20.0h per log.
                    </div>
                  </form>
                </div>
              )}
                            ...
                            <td className="px-6 py-4 text-right">
                              <div className="flex justify-end items-center gap-2">
                                ...
                                <button
                                  ...
                                  className="p-1 text-[#a1a1a1] hover:text-amber-500 transition-colors"
                                  title="Edit Log"
                                >
                                  <Edit2 className="w-3.5 h-3.5" />
                                </button>
                                <button
                                  ...
                                  className="p-1 text-[#a1a1a1] hover:text-red-500 transition-colors"
                                  title="Delete Log"
                                >
            ...
            <div className="space-y-4 max-w-6xl">
              <div className="flex flex-col xl:flex-row xl:items-center justify-between gap-4 bg-[#171717] p-3 rounded-xl border border-[#2e2e2e]">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-[#a1a1a1] text-[10px] font-medium">
                      Goal: 20 verified hours.
                    </p>
                  </div>
                </div>
                <div className="flex flex-col sm:flex-row sm:items-center gap-3">
                  ...
                  <div className="flex items-center gap-2">
                    <button
                      onClick={async () => {
                        const eligible = studentProgress.filter(
                          (s) => s.verifiedHours >= 20 && !s.approval,
                        );
                        if (eligible.length === 0) {
                          showAlert(
                            "Nothing to Approve",
                            "No students currently meet the 20-hour requirement for approval.",
                            "info",
                          );
                          return;
                        }
                        if (
                          !window.confirm(
                            `Batch approve completion for ${eligible.length} eligible students?`,
                          )
                        )
                          return;

                        const adminDoc = members.find(
                          (m) => m.id === user?.uid,
                        );
                        if (!adminDoc || !adminDoc.signature) {
                          showAlert(
                            "Action Required",
                            "You must set up your digital signature before bulk approving.",
                            "warning",
                          );
                          setTab("settings");
                          return;
                        ...
                        try {
                          ...
                          showAlert(
                            "Success",
                            `${eligible.length} students approved successfully.`,
                            "success",
                          );
                          // Result handled by onSnapshot
                        } catch (e) {
                      ...
                      onClick={async () => {
                        ...
                        if (ready.length === 0) {
                          showAlert(
                            ...
                            "No students have been approved for completion yet.",
                            "info",
                          );
                          return;
                        }
                        if (
                          !window.confirm(
                            `Send completion emails to ${ready.length} approved students?`,
                          )
                        )
                          return;
                        ...
                        showAlert(
                          "Sending...",
                          "Batch sending emails in progress. This may take a moment.",
                          "info",
                        );
                        ...
                        for (const student of ready) {
                          try {
                            ...
                            console.error(
                              `Failed to send to ${student.studentEmail}`,
                              e,
                            );
                          }
                        }
                        showAlert(
                          "Success",
                          `Finished sending ${sentCount} emails.`,
                          "success",
                        );
                      }}
              ...
              <div className="grid grid-cols-3 gap-2">
                <div className="bg-[#171717] border border-[#2e2e2e] p-2 rounded-lg flex flex-col items-center">
                  <div className="text-[8px] text-[#a1a1a1] uppercase font-bold tracking-tight mb-0.5">
                    Total
                  </div>
                  <div className="text-sm font-bold">
                    {studentProgressRaw.length}
                  </div>
                </div>
                <div className="bg-[#171717] border border-[#2e2e2e] p-2 rounded-lg flex flex-col items-center">
                  <div className="text-[8px] text-[#a1a1a1] uppercase font-bold tracking-tight mb-0.5">
                  ...
                  <div className="text-sm font-bold text-[#3ecf8e]">
                    {
                      studentProgressRaw.filter((s) => s.verifiedHours >= 20)
                        .length
                    }
                  </div>
                </div>
                ...
                <table className="min-w-full text-left text-sm whitespace-nowrap">
                  <thead className="bg-[#262626] border-b border-[#2e2e2e] text-[#a1a1a1] text-[9px] font-bold uppercase tracking-wider">
                    <tr>
                      ...
                      <th className="px-3 py-2 text-center w-24 md:w-40">
                        Progress
                      </th>
                  ...
                  <tbody className="divide-y divide-[#2e2e2e]">
                    {studentProgress.length === 0 ? (
                      <tr>
                        <td
                          colSpan={6}
                          className="px-6 py-6 text-center text-[#a1a1a1]"
                        >
                      ...
                      studentProgress.map((s) => (
                        <tr key={s.studentNo} className="hover:bg-[#1c1c1c]">
                          <td className="px-3 py-1.5">
                            ...
                            <div className="text-[8px] text-[#3ecf8e] font-mono leading-tight mt-0.5">
                              {s.studentNo}
                            </div>
                          </td>
                          ...
                          <td className="px-3 py-1.5 text-center">
                            ...
                            <div className="text-[8px] text-[#666] italic leading-tight">
                              of {s.totalHours.toFixed(1)}h
                            </div>
                          </td>
                          <td className="px-3 py-1.5">
                            <div className="flex items-center gap-2">
                              <div className="h-1 bg-[#262626] flex-1 rounded-full overflow-hidden border border-white/5">
                                <div
                                  ...
                                  style={{
                                    width: `${Math.min(100, (s.verifiedHours / 20) * 100)}%`,
                                  }}
                                />
                              </div>
                              <span className="text-[8px] text-[#a1a1a1] w-4 text-right font-mono">
                                {Math.round(
                                  Math.min(100, (s.verifiedHours / 20) * 100),
                                )}
                                %
                              </span>
                            </div>
                          ...
                          <td className="px-3 py-1.5 text-right">
                            {s.approval ? (
                              ...
                              <span className="text-[#a1a1a1] text-[8px] font-medium italic">
                                Requirement not met
                              </span>
                            )}
                          </td>
            ...
            <div className="space-y-4 max-w-6xl">
              <div className="flex flex-col md:flex-row md:items-center justify-between gap-3">
                <div>
                  ...
                  <p className="text-[#a1a1a1] text-[10px] mt-0.5">
                    Review completed tasks and corresponding student logs.
                  </p>
                </div>
                <div className="relative w-full md:w-64">
                  <Search className="absolute left-3 top-1/2 -translate-y-1/2 w-3.5 h-3.5 text-[#a1a1a1]" />
                          ...
                          return (
                            <tr key={t.id} className="hover:bg-[#1c1c1c]">
                              <td className="px-4 py-2">
                                ...
                                <div className="text-[8px] text-[#a1a1a1] uppercase font-mono">
                                  {formatDate(t.date)}
                                </div>
                              </td>
                              <td className="px-4 py-2">
                                ...
                                <div className="text-[8px] text-[#a1a1a1]">
                                  {formatTime(t.startTime)} -{" "}
                                  {formatTime(t.endTime)}
                                </div>
                              </td>
                              ...
                              <td className="px-4 py-2">
                                <div className="flex -space-x-1.5">
                                  {executors.slice(0, 4).map((ex, i) => (
                                    <div
                                      key={i}
                                      className="w-7 h-7 rounded-full border border-[#171717] bg-[#2e2e2e] flex items-center justify-center text-[10px] font-bold"
        ...
        <table className="min-w-full text-left text-sm whitespace-nowrap">
          ...
          <tbody className="divide-y divide-[#2e2e2e]">
            {filteredMembers.map((m) => {
              ...
              if (m.lastLogin) {
                const parsed = typeof m.lastLogin === 'object' && (m.lastLogin as any).toDate ? (m.lastLogin as any).toDate() : new Date(m.lastLogin);
              ...
              return (
                <tr key={m.id} className="hover:bg-[#1c1c1c]">
                  ...
                  <td className="px-6 py-4">
                    <div className="flex items-center gap-3">
                      {m.photoURL ? <img src={m.photoURL} alt="" className="w-8 h-8 rounded-full border border-[#2e2e2e]" referrerPolicy="no-referrer" /> : <div className="w-8 h-8 rounded-full bg-[#262626] border border-[#2e2e2e] flex items-center justify-center font-bold text-[#a1a1a1]">{(m.displayName || "?").charAt(0).toUpperCase()}</div>}
                      <div>
                        <div className="font-bold text-[#ededed]">{m.displayName || "User"}</div>
                  ...
                  <td className="px-6 py-4">
                    {(members.find((usr) => usr.id === user?.uid)?.role === "developer" || (members.find((usr) => usr.id === user?.uid)?.role === "admin" && m.role !== "developer")) ? (
                      <select value={m.role} onChange={(e) => handleUpdateRole(m.id, e.target.value as any, m.displayName, m.role)} className="bg-[#1c1c1c] border border-[#2e2e2e] rounded text-xs px-2 py-1 outline-none text-[#ededed]">
                      ...
                      <span className="text-[10px] px-2 py-0.5 rounded font-bold uppercase bg-[#3ecf8e]/10 text-[#3ecf8e]">{m.role?.replace("_", " ")}</span>
                    )}
                  </td>
          ...
          {tab === "settings" && (
            <div className="space-y-8 max-w-4xl">
              <div>
                ...
                <p className="text-[#a1a1a1] text-sm mt-1">
                  Manage your profile information and digital signature.
                </p>
              </div>
